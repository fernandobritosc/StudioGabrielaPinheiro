...
export default function Sidebar() {
  ...
  return (
    <aside className="w-72 bg-white h-screen border-r border-border flex flex-col p-6 shadow-sm shrink-0">
      ...
      <nav className="flex-1 space-y-2">
        {menuItems.map((item) => {
          // Note: The original menuItems array uses 'href' and does not have 'id'.
          // The provided new code uses 'item.path' and 'item.id'.
          // For consistency with the original menuItems, 'item.href' is used here.
          // The Icon selection logic is also simplified to use item.icon directly.
          const isActive = pathname === item.href;
          const IconComponent = item.icon; // Use the icon component directly

          return (
            <Link
              key={item.href} // Using item.href as key
              href={item.href}
